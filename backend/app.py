...
def run_sdxlturbo(pipeline, source_image, control_image, generator, prompt, num_inference_steps, strength, conditioning_scale):
    ...
    try:
        ...
        with latent_lock:
            last_latent = gen_image
        gen_image = gen_image.to('cuda')

        image = pipeline.vae.decode(gen_image / pipeline.vae.config.scaling_factor, return_dict=False)[0]
        image = (image / 2 + 0.5).clamp(0, 1)
        image =  image.permute(
            0, 2, 3, 1
        ).contiguous().float().mul(255).round()
        image = image.to(dtype=torch.uint8).to('cpu')[0]
        image = image.numpy()
